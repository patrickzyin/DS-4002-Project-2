...
output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "OUTPUT", "linear_regression_full")
...
print("=" * 45)
print("   Linear Regression (Full) — Time-Ordered")
print("=" * 45)
print(f"  R-squared : {r2_ordered:.4f}  (goal: >= 0.70)")
print(f"  MAE       : {mae_ordered:.4f}")
print("=" * 45)
print("=" * 45)
print("   Linear Regression (Full) — Shuffled")
print("=" * 45)
...
with open(os.path.join(output_dir, "linear_regression_full_results.txt"), "w", encoding="utf-8") as f:
    f.write("Linear Regression — Full Dataset (1980–2026)\n")
    f.write("=" * 45 + "\n")
...
plt.title("Linear Regression (Full): Actual vs Predicted (Time-Ordered)")
plt.xlabel("Month (test set index)")
plt.ylabel("UMCSENT")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "linear_full_actual_vs_predicted.png"), dpi=150, bbox_inches="tight")
plt.show()

plt.figure(figsize=(12, 4))
plt.plot(y_test_s.values, label="Actual", linewidth=2)
plt.plot(y_pred_s, label="Predicted", linewidth=2, linestyle="--")
plt.title("Linear Regression (Full): Actual vs Predicted (Shuffled)")
plt.xlabel("Observation index")
plt.ylabel("UMCSENT")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "linear_full_actual_vs_predicted_shuffled.png"), dpi=150, bbox_inches="tight")
plt.show()
...
plt.title("Standardized Coefficients – Linear Regression (Full)")
plt.xlabel("Coefficient Value")
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "linear_full_coefficients.png"), dpi=150, bbox_inches="tight")
plt.show()