...
#Output folder
output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "OUTPUT", "lasso_precovid")
...
#Fit Lasso with time series cross validation
...
#Print results
print("=" * 20)
print("Lasso (Pre-COVID) — Time-Ordered Split")
print("=" * 20)
...
print("=" * 20)
print("=" * 20)
print("Lasso (Pre-COVID) — Shuffled Split")
print("=" * 20)
...
#Save results
with open(os.path.join(output_dir, "lasso_precovid_results.txt"), "w", encoding="utf-8") as f:
    f.write("Lasso Regression — Pre-COVID (1980–March 2020)\n")
    f.write("=" * 20 + "\n")
...
plt.title("Lasso (Pre-COVID): Actual vs Predicted (Time-Ordered)")
plt.xlabel("Month (test set index)")
plt.ylabel("UMCSENT")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "lasso_precovid_actual_vs_predicted.png"), dpi=150, bbox_inches="tight")
plt.show()
...
plt.title("Lasso (Pre-COVID): Actual vs Predicted (Shuffled)")
plt.xlabel("Observation index")
plt.ylabel("UMCSENT")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "lasso_precovid_actual_vs_predicted_shuffled.png"), dpi=150, bbox_inches="tight")
plt.show()
...
plt.title("Lasso: Non-Zero Coefficients (Pre-COVID)")
plt.xlabel("Coefficient Value")
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "lasso_precovid_coefficients.png"), dpi=150, bbox_inches="tight")
plt.show()